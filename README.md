...
| `scan-ref`                   | String  | `.`                                | Scan reference, e.g. `.` or `/github/workspace/`                                                                                                               |
...
| `hide-progress`              | String  |                                    | Suppress progress bar and log output                                                                                                                        |
| `list-all-pkgs`              | String  | `false`                            | Output all packages regardless of vulnerability                                                                                                         |
...
| `version`                    | String  | `v0.71.2`                          | Trivy version to use, e.g. `latest` or `v0.71.2`                                                                                                                 |
| `cache`                      | Boolean | `true`                             | Enable caching of vulnerability DB, Java DB, and checks bundle. Set to `false` to disable. |
| `skip-setup-trivy`           | Boolean | false                              | Skip calling the `setup-trivy` action to install `trivy`                                                                                                         |
| `token-setup-trivy`          | String |                                    | Overwrite `github.token` used by `setup-trivy` to checkout the `trivy` repository                                                                                |
