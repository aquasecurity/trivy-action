...
> **Security Notice:** Always pin this action to a full 40-character commit SHA, not a version tag.
> Version tags (e.g. `@v0.36.0`, `@master`) are mutable — any actor with write access to this repo can
> silently redirect them to malicious code via a force-push, without your workflow YAML ever changing.
> Use `aquasecurity/trivy-action@<sha> # vX.Y.Z` to pin immutably while keeping the version readable.
> Run [`pinact run`](https://github.com/suzuki-shunsuke/pinact) to migrate all workflows in your repo automatically.
